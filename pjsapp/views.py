...
from .models import Team,Department,Mentor,Project
...
def checkemplogin(request):
    ...
    flag = Team.objects.filter(Q(username=uname) & Q(password=pwd))

    print(flag)

    if flag:
        emp = Team.objects.get(username=uname)
        print(emp)
...
def empupdatepwd(request):
    ...
    flag = Team.objects.filter(Q(id=eid) & Q(password=opwd))

    if flag:
        Team.objects.filter(id=eid).update(password=npwd)
...
def vieweproducts(request):
    ...
    productlist = Project.objects.all()
...
def displayeproducts(request):
    ...
    productlist = Project.objects.filter(name__icontains=pname)
...
def checkadminlogin(request):
    ...
    flag = Mentor.objects.filter(Q(username__exact=uname) & Q(password__exact=pwd))
    print(flag)

    if flag:
        admin = Mentor.objects.get(username=uname)
        print(admin)
...
def deleteemp(reequest,eid):
    Team.objects.filter(id=eid).delete()
